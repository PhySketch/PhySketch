...
import physketch.masks as masks
import cfg
import os
import binascii
...
class SampleCropper:
    ...
    def setMask(self):
        ...
        self.cropMask = ps.CropMask(copy.deepcopy(masks.CROP_MASK[chr(atividade)]),self.imageOriginal.shape[1],self.imageOriginal.shape[0])
    ...
    def click_and_drag(self, event, x, y, flags, param):
        if self.cropMask != '':
            if event == cv.EVENT_LBUTTONDOWN:
                #print("BT DOWN")
                ls = self.cropMask.find_closest_lines(ps.Point(x,y),10)

                #print((x,y),ls)
                self.selected_lines = ls

            elif event == cv.EVENT_MOUSEMOVE:
                #print("MOVE")
                for quad, linha in self.selected_lines:
                    self.cropMask.move_line(quad,linha, ps.Point(x, y))
    ...
    def crop(self):
        ...
        if height > width:
            self.imageOriginal = ps.rotate_bound(self.imageOriginal, 90)
        ...
        while True:
            ...
            if key == ord("c"):
                if self.cropped == True:
                    ...
                    mask_out = [ps.QuadMask(ps.Point(0,0),ps.Point(0,0))]
            ...
            if key == ord("s") and self.cropped:
                for i, image in enumerate(self.crop_image_list):
                    path = os.path.join(cfg.OUTPUT_DIR,self.filename+'_'+str(i)+'.png')
    ...
    def __init__(self):
        ...
        for item in sorted(os.listdir(cfg.INPUT_DIR)):
            if not item.startswith('.') and os.path.isfile(os.path.join(cfg.INPUT_DIR, item)):
                self.listSamples.append(SampleCropper(os.path.join(cfg.INPUT_DIR, item)))
